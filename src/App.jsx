...
const V={DASH:"dash",OYENTES:"oy",OY_DET:"oy_det",OY_NEW:"oy_new",OY_QUICK:"oy_quick",OY_EDIT:"oy_edit",CONS:"con",CON_DET:"con_det",CON_NEW:"con_new",CON_EDIT:"con_edit",SORTEO:"sorteo",BUSCAR:"buscar",STATS:"stats"};

const daysSince=d=>d?Math.floor((Date.now()-new Date(d).getTime())/86400000):null;
const isRecent=o=>{const d=daysSince(o?.ultimoWin);return d!==null&&d<=30;};
const uid=()=>Math.random().toString(36).slice(2,10);
const today=()=>new Date().toISOString().split("T")[0];
const daysUntil=d=>{if(!d)return null;const diff=Math.ceil((new Date(d)-Date.now())/86400000);return diff;};
...
function LoginScreen({onLogin}){
  ...
  const attempt=()=>{
    if(pass===ACCESS_PASSWORD){onLogin("admin");}
    else if(pass===PRODUCER_PASSWORD){onLogin("producer");}
  ...
  return(
    <div style={{minHeight:"100vh",background:"#F0F2F7",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:24,fontFamily:"'Nunito',sans-serif"}}>
      ...
      <div className={`card${shake?" shake":""} fade-in`} style={{padding:32,width:"100%",maxWidth:360}}>
        ...
        <p style={{color:C.muted,fontSize:11,textAlign:"center",marginTop:12}}>Dos niveles de acceso disponibles</p>
      </div>
    </div>
  );
}
...
/* ─── MAIN APP ── */
function MainApp({onLogout,role}){
  ...
  const [waMsg,setWaMsg]=useState(null); // mensaje whatsapp generado
  // Filtros oyentes
  ...
  const addLog=useCallback((msg)=>setLog(p=>[{msg,hora:new Date().toLocaleTimeString("es-AR",{hour:"2-digit",minute:"2-digit"})},...p].slice(0,50)),[]);
  ...
  const conGanadores=useCallback((conId)=>participaciones.filter(p=>p.concursoId===conId&&p.esGanador==="si").map(p=>({...oyentes.find(o=>o.id===p.oyenteId),_qty:p.cantGanada||"1",_fecha:p.fecha})).filter(o=>o.id),[participaciones,oyentes]);
  ...
  // Stats demográficas de participantes de un concurso
  const conStats=useCallback((conId)=>{
    ...
    const topLocs=Object.entries(localidades).sort((a,b)=>b[1]-a[1]).slice(0,5);
    return{gen,rangos,topLocs,total:parts.length};
  },[conPartic]);
  ...
  const addOyente=async()=>{
    ...
    try{
      ...
      for(const conId of oyConIds){
        ...
        const p={concursoId:conId,oyenteId:newO.id,fecha:today(),concursoNombre:con.nombre||"",concursoPremio:con.premio||"",esGanador:con.tipo==="ganador_directo"?"si":"no",cantGanada:"1"};
        await api("addParticipante",p);setPartic(prev=>[...prev,p]);inscriptos++;
        if(con.tipo==="ganador_directo"){
          const nw=(parseInt(newO.totalWins)||0)+ganados+1;
          const su=(parseInt(con.stockUsado)||0)+1;
          await api("setGanadorDirecto",{concursoId:conId,oyenteId:newO.id,oyenteNombre:newO.nombre,newTotalWins:nw,ultimoWin:today(),qty:1,stockUsado:su});
      ...
      addLog(`Nuevo oyente: ${newO.nombre}${ganados>0?" · ganador en "+ganados+" concurso(s)":""}`);
      showToast(inscriptos>0?`Oyente registrado en ${inscriptos} concurso${inscriptos>1?"s":""}  ✓`:"Oyente registrado ✓");
      nav(V.OYENTES);
  ...
  const addQuick=async()=>{
    ...
    setSaving(true);
    try{
      const newO={id:uid(),...quickForm,dni:"",email:"",edad:"",localidad:"",genero:"",nota:"",fechaRegistro:today(),veces:0,totalWins:0,ultimoWin:""};
      await api("addOyente",newO);
      setOyentes(p=>[...p,newO]);setQuickForm(emptyQuick);
      addLog(`Registro rápido: ${newO.nombre}`);
      showToast("Oyente registrado rápidamente ✓");
      nav(V.OY_DET,newO.id);
    }catch(e){setFormErr("Error: "+e.message);}
    finally{setSaving(false);}
  };
    ...
    try{
      ...
      if(editConId){
        ...
        if(con&&!inCon(selId,editConId)){
          const p={concursoId:editConId,oyenteId:selId,fecha:today(),concursoNombre:con.nombre||"",concursoPremio:con.premio||"",esGanador:con.tipo==="ganador_directo"?"si":"no",cantGanada:"1"};
          await api("addParticipante",p);setPartic(prev=>[...prev,p]);
          if(con.tipo==="ganador_directo"){
            const nw=(parseInt(updated.totalWins)||0)+1;
            const su=(parseInt(con.stockUsado)||0)+1;
            await api("setGanadorDirecto",{concursoId:editConId,oyenteId:selId,oyenteNombre:updated.nombre,newTotalWins:nw,ultimoWin:today(),qty:1,stockUsado:su});
  ...
  const addConcurso=async()=>{
    ...
    try{
      ...
      addLog(`Nuevo concurso: ${newC.nombre}`);
  ...
  const updateConcurso=async()=>{
    ...
    try{
      ...
      addLog(`Edición concurso: ${updated.nombre}`);
  ...
  const inscribir=async(oyId,conId)=>{
    ...
    try{
      const p={concursoId:conId,oyenteId:oyId,fecha:today(),concursoNombre:con?.nombre||"",concursoPremio:con?.premio||"",esGanador:esGD?"si":"no",cantGanada:"1"};
      await api("addParticipante",p);setPartic(prev=>[...prev,p]);
      if(esGD){
        const oy=oyentes.find(o=>o.id===oyId);
        const nw=(parseInt(oy?.totalWins)||0)+1;
        const su=(parseInt(con?.stockUsado)||0)+1;
        await api("setGanadorDirecto",{concursoId:conId,oyenteId:oyId,oyenteNombre:oy?.nombre||"",newTotalWins:nw,ultimoWin:today(),qty:1,stockUsado:su});
        setConcursos(prev=>prev.map(c=>c.id===conId?{...c,stockUsado:su}:c));
        setOyentes(prev=>prev.map(o=>o.id===oyId?{...o,veces:(parseInt(o.veces)||0)+1,totalWins:nw,ultimoWin:today()}:o));
        addLog(`🏆 Ganador: ${oy?.nombre} en ${con?.nombre}`);
  ...
  const marcarGanadorDirecto=async(oyente,qty=1)=>{
    ...
    try{
      // Actualizar esGanador en participacion existente
      await api("setGanadorDirecto",{concursoId:selCon.id,oyenteId:oyente.id,oyenteNombre:oyente.nombre,newTotalWins:nw,ultimoWin:today(),qty,stockUsado:su});
      // Marcar esGanador en participaciones locales
      ...
      const msg=`¡Hola ${oyente.nombre}! 🎉 ¡Felicitaciones! Ganaste ${qty>1?qty+" entradas":"una entrada"} para ${selCon.nombre} (${selCon.premio}). Comunicate con nosotros para coordinar la entrega. — Metro Power Hits 📻`;
      setWaMsg(msg);
      addLog(`🏆 Ganador: ${oyente.nombre} (${qty}x) en ${selCon.nombre}`);
      if(sinStock)showToast(`🏆 ${oyente.nombre} · ⚠️ ¡Stock agotado!`,"win");
      else showToast(`🏆 ${oyente.nombre} ganó ${qty} entrada${qty>1?"s":""}`, "win");
    }catch(e){showToast("Error: "+e.message,"err");}
  };

  const archivarConcurso=async(conId)=>{
    try{await api("archivarConcurso",{concursoId:conId});setConcursos(p=>p.map(c=>c.id===conId?{...c,estado:"archivado"}:c));addLog("Concurso archivado: "+concursos.find(c=>c.id===conId)?.nombre);showToast("Concurso archivado");nav(V.CONS);}
    catch(e){showToast("Error: "+e.message,"err");}setConfirm(null);
  };

  const reactivarConcurso=async(conId)=>{
    try{await api("reactivarConcurso",{concursoId:conId});setConcursos(p=>p.map(c=>c.id===conId?{...c,estado:"activo"}:c));showToast("Concurso reactivado ✓");}
    catch(e){showToast("Error: "+e.message,"err");}
  };

  const extenderConcurso=async(conId,nf)=>{
    if(!nf){showToast("Seleccioná una fecha","err");return;}
    try{await api("extenderConcurso",{concursoId:conId,fechaHasta:nf});setConcursos(p=>p.map(c=>c.id===conId?{...c,fechaHasta:nf}:c));setExtendiendo(false);setNuevaFecha("");showToast("Extendido hasta "+fmtDate(nf)+" ✓");}
    catch(e){showToast("Error: "+e.message,"err");}
  };
  ...
  const eliminarConcurso=async(conId)=>{
    ...
    try{await api("eliminarConcurso",{concursoId:conId});setConcursos(p=>p.filter(c=>c.id!==conId));addLog("Eliminado concurso id="+conId);showToast("Concurso eliminado");nav(V.CONS);}
    catch(e){showToast("Error: "+e.message,"err");}setConfirm(null);
  };
  ...
  const realizarSorteo=async()=>{
    ...
    const iv=setInterval(()=>{
      ...
      if(tick>25){
        clearInterval(iv);
        const shuffled=[...partic].sort(()=>Math.random()-.5);
        ...
        winners.forEach((winner,i)=>{
          ...
          api("setGanador",{concursoId:selCon.id,oyenteId:winner.id,oyenteNombre:winner.nombre,newTotalWins:nw,ultimoWin:today(),esSorteoMultiple:n>1});
          ...
          addLog(`🎲 Sorteo ${selCon.nombre}: ganó ${winner.nombre}`);
        });
  ...
  const exportExcelOyentes=async()=>{
    ...
    try{
      ...
      const COLS=[{h:"Nombre",k:"nombre",w:28},{h:"Teléfono",k:"telefono",w:18},{h:"DNI",k:"dni",w:13},{h:"Email",k:"email",w:26},{h:"Edad",k:"edad",w:8},{h:"Localidad",k:"localidad",w:20},{h:"Género",k:"genero",w:10},{h:"Particip.",k:"veces",w:12},{h:"Premios",k:"totalWins",w:12},{h:"Últ.Premio",k:"ultimoWin",w:14},{h:"Nota",k:"nota",w:24}];
      ...
      oyentes.forEach((o,i)=>{
        const row=ws.addRow({nombre:o.nombre||"",telefono:o.telefono||"",dni:o.dni||"",email:o.email||"",edad:parseInt(o.edad)||"",localidad:o.localidad||"",genero:o.genero||"",veces:parseInt(o.veces)||0,totalWins:parseInt(o.totalWins)||0,ultimoWin:o.ultimoWin||"",nota:o.nota||""});
  ...
  const exportExcelConcurso=async(conId)=>{
    ...
    try{
      ...
      ws.mergeCells("A1:G1");const tc=ws.getCell("A1");tc.value=`${con.nombre} — ${con.premio}`;tc.font={name:"Arial",bold:true,size:13,color:{argb:"FFFFFFFF"}};tc.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF10B981"}};tc.alignment={horizontal:"center",vertical:"middle"};ws.getRow(1).height=28;
      const COLS=[{h:"Nombre",k:"nombre",w:28},{h:"Teléfono",k:"telefono",w:18},{h:"DNI",k:"dni",w:13},{h:"Email",k:"email",w:24},{h:"Localidad",k:"localidad",w:18},{h:"Premios recibidos",k:"_qty",w:16},{h:"Fecha",k:"_fecha",w:14}];
      ws.columns=COLS.map(c=>({key:c.k,width:c.w}));
      const hr=ws.getRow(2);COLS.forEach((c,i)=>{const cell=hr.getCell(i+1);cell.value=c.h;cell.font={name:"Arial",bold:true,size:10,color:{argb:"FFFFFFFF"}};cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:"FF1E3A5F"}};cell.alignment={horizontal:"center",vertical:"middle"};});hr.height=22;
      const all=conPartic(conId).map(o=>{const par=participaciones.find(p=>p.concursoId===conId&&p.oyenteId===o.id);return{...o,_qty:par?.cantGanada||"1",_fecha:par?.fecha||""};});
      all.forEach((o,i)=>{
        const row=ws.addRow({nombre:o.nombre||"",telefono:o.telefono||"",dni:o.dni||"",email:o.email||"",localidad:o.localidad||"",_qty:parseInt(o._qty)||1,_fecha:o._fecha||""});
        const bg=i%2===0?"FFFFFFFF":"FFECFDF5";
        row.eachCell(cell=>{cell.font={name:"Arial",size:10};cell.fill={type:"pattern",pattern:"solid",fgColor:{argb:bg}};cell.alignment={vertical:"middle"};cell.border={bottom:{style:"hair",color:{argb:"FFDDE6F0"}}};});
        row.height=18;
      });
      const sr=all.length+4;ws.getCell(`A${sr}`).value=`Total participantes: ${all.length}`;ws.getCell(`A${sr}`).font={name:"Arial",bold:true,size:11};
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Nunito',sans-serif",color:C.text}}>
      ...
      <div style={{background:C.headerBg,borderBottom:`1px solid ${C.border}`,padding:"10px 16px",display:"flex",alignItems:"center",gap:8,position:"sticky",top:0,zIndex:200,boxShadow:C.shadow,flexWrap:"wrap"}}>
        ...
        {!isAdmin&&<span style={{background:C.warn+"22",color:C.warn,border:`1px solid ${C.warn}44`,borderRadius:4,padding:"2px 8px",fontSize:10,fontWeight:700}}>PRODUCTOR</span>}
      ...
      <div style={{padding:"20px 16px",maxWidth:960,margin:"0 auto"}}>
        ...
        {view===V.STATS&&(
          <div className="fade-in">
              ...
              <div className="card" style={{padding:18}}>
                ...
                {consActivos.map(c=>(
                  <div key={c.id} className="row" onClick={()=>nav(V.CON_DET,c.id)} style={{padding:"9px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                    ...
                    <div style={{color:C.muted,fontSize:11,marginTop:1,display:"flex",gap:8,alignItems:"center"}}>{conPartic(c.id).length} participantes · {c.tipo==="sorteo"?"🎲":"🏆"} <Countdown fechaHasta={c.fechaHasta}/></div>
                  </div>
                ))}
              </div>
            </div>
            ...
            {log.length>0&&(
              <div className="card" style={{padding:18}}>
                <h3 style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:15,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>Actividad reciente (esta sesión)</h3>
                <div style={{maxHeight:220,overflowY:"auto"}}>
                  {log.map((l,i)=>(
                    <div key={i} style={{display:"flex",justifyContent:"space-between",padding:"6px 0",borderBottom:`1px solid ${C.border}`,fontSize:12}}>
                      ...
                      <span style={{color:C.muted,flexShrink:0,marginLeft:8}}>{l.hora}</span>
                    </div>
                  ))}
                </div>
              </div>
        ...
        {view===V.OY_QUICK&&(
          <div className="fade-in" style={{maxWidth:440}}>
            ...
            <p style={{color:C.muted,fontSize:11,textAlign:"center",marginTop:10}}>Vas a ir directo al perfil para completar más datos o inscribir en concursos.</p>
          </div>
        )}
          ...
          <div className="fade-in">
            ...
            {mostrarFiltros&&(
              <div className="card fade-in" style={{padding:14,marginBottom:12,display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:10}}>
                <div>
                  <Lbl>Género</Lbl>
                  <select className="sel" value={filtroGenero} onChange={e=>setFiltroGenero(e.target.value)} style={{padding:"7px 10px",fontSize:12}}>
                    <option value="">Todos</option><option>Mujer</option><option>Hombre</option><option>Otro</option>
                  </select>
                </div>
                <div>
                  <Lbl>Localidad</Lbl>
                  <select className="sel" value={filtroLocalidad} onChange={e=>setFiltroLocalidad(e.target.value)} style={{padding:"7px 10px",fontSize:12}}>
                ...
                <div>
                  <Lbl>Edad</Lbl>
                  <select className="sel" value={filtroEdad} onChange={e=>setFiltroEdad(e.target.value)} style={{padding:"7px 10px",fontSize:12}}>
                    <option value="">Todas</option><option value="18-25">18-25</option><option value="26-35">26-35</option><option value="36-45">36-45</option><option value="46+">46+</option>
                  </select>
                </div>
                <div style={{display:"flex",alignItems:"flex-end",paddingBottom:2}}>
                  <label style={{display:"flex",alignItems:"center",gap:6,cursor:"pointer",fontSize:12,color:C.text,fontWeight:600}}>
                    <input type="checkbox" checked={filtroReciente} onChange={e=>setFiltroReciente(e.target.checked)} style={{width:14,height:14}}/>
                    Ganó en 30d
                  </label>
                </div>
        ...
        {view===V.OY_NEW&&(
          <div className="fade-in" style={{maxWidth:520}}>
            ...
            <div style={{marginTop:14}}>
              ...
              <textarea className="inp" placeholder="Observaciones sobre el oyente, preferencias, etc..." value={oyForm.nota} onChange={e=>setOyForm(p=>({...p,nota:e.target.value}))}/>
            </div>
          ...
          <div className="fade-in">
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {consVisibles.map(c=>{
                ...
                const du=daysUntil(c.fechaHasta);
                return(
          ...
          <div className="fade-in">
            ...
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:10}}>
              ...
              <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                ...
                {selCon.estado==="activo"&&(
                  <button className="btn-ghost" style={{fontSize:12}} onClick={()=>{setEditConForm({nombre:selCon.nombre||"",premio:selCon.premio||"",descripcion:selCon.descripcion||"",fechaDesde:selCon.fechaDesde||"",fechaHasta:selCon.fechaHasta||"",tipo:selCon.tipo||"ganador_directo",stockTotal:selCon.stockTotal||""});setFormErr("");nav(V.CON_EDIT,selCon.id);}}>✏️ Editar</button>
                )}
                ...
                {selCon.estado!=="archivado"&&isAdmin&&(
            ...
            {selCon.ganadorNombre&&(
              <div style={{background:C.green+"11",border:`1px solid ${C.green}44`,borderRadius:10,padding:"14px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:12}}>
                ...
                <div><div style={{color:C.green,fontWeight:700,fontSize:18,fontFamily:"'Barlow Condensed',sans-serif",textTransform:"uppercase",letterSpacing:1}}>{selCon.tipo==="ganador_directo"?"Último ganador:":"Ganador:"} {selCon.ganadorNombre}</div><div style={{color:C.muted,fontSize:12}}>Premio: {selCon.premio}</div></div>
              </div>
            )}

            {/* Mensaje WhatsApp generado */}
            {waMsg&&(
              <div className="card fade-in" style={{padding:"14px 18px",marginBottom:16,border:`2px solid ${C.green}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                  <span style={{fontWeight:700,fontSize:13,color:C.green}}>📱 Mensaje para WhatsApp</span>
                  <button className="btn-ghost" style={{padding:"3px 10px",fontSize:11}} onClick={()=>setWaMsg(null)}>✕</button>
                </div>
                <div style={{background:C.bg,borderRadius:6,padding:"10px 12px",fontSize:13,color:C.text,lineHeight:1.6,marginBottom:10}}>{waMsg}</div>
                <button className="btn-green" style={{padding:"6px 16px",fontSize:12}} onClick={()=>{navigator.clipboard.writeText(waMsg);showToast("Copiado al portapapeles ✓");}}>📋 Copiar mensaje</button>
              </div>
            )}
            ...
            {extendiendo&&(
              <div className="card fade-in" style={{padding:"14px 18px",marginBottom:16,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
                <span style={{fontWeight:600,fontSize:13}}>📅 Nueva fecha límite:</span>
              ...
              return(
                <div className="card" style={{padding:"14px 18px",marginBottom:16}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8,flexWrap:"wrap",gap:8}}>
                    <div><span style={{fontWeight:700,fontSize:13,color:agotado?C.red:C.text}}>🎁 Stock: </span><span style={{fontWeight:800,fontSize:15,color:agotado?C.red:C.green}}>{restantes}</span><span style={{color:C.muted,fontSize:12}}> de {total} disponibles · {usados} entregados</span></div>
                    {agotado&&selCon.estado==="activo"&&(
                      <button className="btn-ghost" style={{fontSize:12,padding:"4px 12px"}} onClick={()=>{const extra=prompt("¿Cuántos premios más?","10");if(extra&&parseInt(extra)>0){const nT=total+parseInt(extra);api("extenderStock",{concursoId:selCon.id,stockTotal:nT});setConcursos(p=>p.map(c=>c.id===selCon.id?{...c,stockTotal:nT}:c));showToast("Stock ampliado a "+nT+" ✓");}}}>➕ Agregar más</button>
                    )}
                  </div>
            ...
            {pickingWinner&&(
              <div className="card fade-in" style={{padding:"16px 18px",marginBottom:16,border:`2px solid ${C.green}`}}>
                ...
                {(()=>{
                  const total=parseInt(selCon.stockTotal)||0;
                  const usados=parseInt(selCon.stockUsado)||0;
                  const restantes=total>0?total-usados:20;
                  ...
                  return(
                    <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                      <div>
                        ...
                        <div style={{color:C.muted,fontSize:11}}>{o.telefono}{par?.cantGanada&&parseInt(par.cantGanada)>1?" · "+par.cantGanada+"x":""}</div>
                      </div>
                      <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
                        ...
                        {selCon.estado==="activo"&&selCon.tipo==="ganador_directo"&&(
                          pickingWinner?.id===o.id?null:<button className="btn-green btn-sm" onClick={()=>{setPickingWinner(o);setCantGanada(1);}}>🏆 Premio</button>
                        )}
                      </div>
                    </div>
                  );
                    ...
                    <div style={{maxHeight:220,overflowY:"auto"}}>
                      {oyentes.filter(o=>!inCon(o.id,selCon.id)).filter(o=>!conBusq||o.nombre.toLowerCase().includes(conBusq.toLowerCase())||o.telefono.includes(conBusq)).map(o=>(
                        <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                          <div>
                            <div style={{fontWeight:600,fontSize:13}}>{o.nombre}</div>
                            <div style={{color:isRecent(o)?C.warn:C.muted,fontSize:11}}>{o.telefono}{isRecent(o)?" · ⚠ "+daysSince(o.ultimoWin)+"d":""}</div>
                          </div>
                  ...
                  return(
                    <div className="card" style={{padding:18}}>
                      ...
                      <div style={{marginBottom:12}}>
                        <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Género</div>
                        {Object.entries(cs.gen).filter(([,v])=>v>0).map(([g,n])=>{
                          const col=g==="Mujer"?"#EC4899":g==="Hombre"?"#3B82F6":g==="Otro"?"#8B5CF6":C.muted;
                          return(<div key={g} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:2}}><span style={{color:C.muted}}>{g}</span><span style={{fontWeight:700}}>{n}</span></div><Bar val={n} max={cs.total} color={col}/></div>);
                        })}
                      </div>
                      <div style={{marginBottom:12}}>
                        <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Edad</div>
                        {Object.entries(cs.rangos).filter(([,v])=>v>0).map(([r,n])=>(
                          <div key={r} style={{marginBottom:6}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:2}}><span style={{color:C.muted}}>{r}</span><span style={{fontWeight:700}}>{n}</span></div><Bar val={n} max={cs.total} color={C.accent}/></div>
                        ))}
                      </div>
                      {cs.topLocs.length>0&&(
                        <div>
                          <div style={{fontSize:11,color:C.muted,fontWeight:700,textTransform:"uppercase",marginBottom:6}}>Top localidades</div>
                          {cs.topLocs.map(([l,n])=>(
                            <div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:12,padding:"3px 0",borderBottom:`1px solid ${C.border}`}}>
                              <span style={{color:C.muted}}>{l}</span><span style={{fontWeight:700}}>{n}</span>
                            </div>
                          ))}
                        </div>
                      )}
        ...
        {view===V.BUSCAR&&(
          <div className="fade-in">
            ...
            {busqueda.length>=4&&(()=>{
              ...
              if(!found)return(<div className="card" style={{padding:28,textAlign:"center"}}><div style={{fontSize:32,marginBottom:10}}>🔍</div><p style={{color:C.muted,fontSize:14}}>No encontrado. <button className="btn" onClick={()=>nav(V.OY_QUICK)} style={{padding:"8px 16px",marginLeft:8,fontSize:13}}>⚡ Registrar rápido</button><button className="btn-ghost" onClick={()=>nav(V.OY_NEW)} style={{padding:"8px 16px",marginLeft:8,fontSize:13}}>+ Completo</button></p></div>);
              return(
...
export default function App(){
  ...
  return loggedIn
    ?<MainApp onLogout={()=>setLoggedIn(false)} role={role}/>

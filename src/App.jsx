...
const LIGHT = {
  ...
  rowHover:"#F8FAFC", headerBg:"#FFFFFF", navBg:"#FFFFFF",
  inputBg:"#F8FAFC", shadow:"0 1px 3px rgba(0,0,0,.08)",
};
const DARK = {
  ...
  rowHover:"#ffffff07", headerBg:"#0D1017", navBg:"#0D1017",
  inputBg:"#07090D", shadow:"none",
};
...
const makeCSS = C => `
  ...
  *{box-sizing:border-box;margin:0;padding:0}
  ...
  .sel{background:${C.inputBg};border:1px solid ${C.border};color:${C.text};border-radius:8px;padding:10px 14px;font-size:14px;width:100%;transition:border-color .2s;appearance:none;cursor:pointer}
  ...
  .btn-green{background:${C.green};color:#fff;border:none;border-radius:8px;padding:11px 22px;font-family:'Barlow Condensed',sans-serif;font-size:15px;font-weight:700;letter-spacing:1px;text-transform:uppercase;transition:opacity .15s}
  ...
  .tag-done{background:${C.border};color:${C.muted};border:1px solid ${C.border};border-radius:5px;padding:2px 8px;font-size:11px;font-weight:700}
...
function MainApp({ onLogout }) {
  ...
  const loadAll = useCallback(async()=>{
    setLoading(true);
    try {
      const d = await api(GAS_URL,"getAll");
  ...
  /* nav */
  const nav = (v,id=null)=>{ setView(v);setSelId(id);setFormErr("");setBusqueda("");setSorteo({animando:false,ganador:null,nombres:[]}); };
  ...
  const conPartic = (conId) => participaciones.filter(p=>p.concursoId===conId).map(id=>oyentes.find(o=>o.id===id?.oyenteId)).filter(Boolean);
  const inCon = (oyId,conId) => participaciones.some(p=>p.oyenteId===oyId&&p.concursoId===conId);

  /* stats */
  const stats = useMemo(()=>{
    const total     = oyentes.length;
    const ganadores = oyentes.filter(o=>parseInt(o.totalWins)>0).length;
    ...
    return {total,ganadores,recentW,promEdad,rangos,generos,consActivos};
  },[oyentes,concursos]);
  ...
  const addOyente = async()=>{
    ...
    try {
      ...
      await api(GAS_URL,"addOyente",newO);
      setOyentes(p=>[...p,newO]);
      setOyForm(emptyOy);
  ...
  const addConcurso = async()=>{
    ...
    try {
      ...
      await api(GAS_URL,"addConcurso",newC);
      setConcursos(p=>[...p,newC]);
      setConForm(emptyCon);
  ...
  const inscribir = async(oyId,conId)=>{
    ...
    try {
      ...
      await api(GAS_URL,"addParticipante",p);
  ...
  const marcarGanadorDirecto = async(oyente)=>{
    ...
    await api(GAS_URL,"setGanador",{concursoId:selCon.id,oyenteId:oyente.id,oyenteNombre:oyente.nombre,newTotalWins:newWins,ultimoWin:today()});
  ...
  const realizarSorteo = async()=>{
    ...
    const iv=setInterval(()=>{
      ...
      if(tick>25){
        ...
        api(GAS_URL,"setGanador",{concursoId:selCon.id,oyenteId:winner.id,oyenteNombre:winner.nombre,newTotalWins:newWins,ultimoWin:today()});
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Nunito',sans-serif",color:C.text}}>
      ...
      {/* HEADER */}
      <div style={{background:C.headerBg,borderBottom:`1px solid ${C.border}`,padding:"10px 20px",display:"flex",alignItems:"center",gap:12,position:"sticky",top:0,zIndex:200,boxShadow:C.shadow}}>
        ...
        <button onClick={()=>setDark(d=>!d)} style={{background:C.bg,border:`1px solid ${C.border}`,borderRadius:20,padding:"4px 12px",color:C.muted,fontSize:12,fontWeight:600}}>
          {dark?"☀️ Claro":"🌙 Oscuro"}
        </button>
      ...
      {/* NAV */}
      <div style={{background:C.navBg,borderBottom:`1px solid ${C.border}`,display:"flex",padding:"0 16px",overflowX:"auto"}}>
        {navTabs.map(n=>(
      ...
      <div style={{padding:"20px 16px",maxWidth:920,margin:"0 auto"}}>
        ...
        {view===V.DASH&&(
          <div className="fade-in">
            ...
            {stats.recentW>0&&(
              <div style={{background:C.warn+"11",border:`1px solid ${C.warn}44`,borderRadius:10,padding:"12px 16px",marginBottom:20,display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:18}}>⚠️</span>
                <span style={{color:C.warn,fontSize:13,fontWeight:600}}>{stats.recentW} oyente{stats.recentW>1?"s":""} ganaron un premio en los últimos 30 días.</span>
              </div>
            )}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:14,flexWrap:"wrap"}}>
              ...
              <div className="card" style={{padding:18}}>
                ...
                {Object.entries(stats.rangos).map(([r,n])=>{
                  const max=Math.max(...Object.values(stats.rangos),1);
                  return(<div key={r} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:2}}><span style={{color:C.muted}}>{r} años</span><span style={{fontWeight:700}}>{n}</span></div><Bar val={n} max={max} color={C.accent}/></div>);
                })}
              </div>
              {/* Género */}
              <div className="card" style={{padding:18}}>
                ...
                {Object.entries(stats.generos).map(([g,n])=>{
                  const max=Math.max(...Object.values(stats.generos),1);
                  const col=g==="Mujer"?"#EC4899":g==="Hombre"?"#3B82F6":"#8B5CF6";
                  return(<div key={g} style={{marginBottom:9}}><div style={{display:"flex",justifyContent:"space-between",fontSize:12,marginBottom:2}}><span style={{color:C.muted}}>{g}</span><span style={{fontWeight:700}}>{n}</span></div><Bar val={n} max={max} color={col}/></div>);
                })}
              ...
              <div className="card" style={{padding:18}}>
                ...
                {concursos.filter(c=>c.estado==="activo").map(c=>(
                  <div key={c.id} className="row" onClick={()=>nav(V.CON_DET,c.id)} style={{padding:"8px 0",borderBottom:`1px solid ${C.border}`,cursor:"pointer"}}>
                ...
                <table style={{width:"100%",borderCollapse:"collapse",minWidth:500}}>
                  <thead>
                    <tr style={{borderBottom:`1px solid ${C.border}`,background:C.bg}}>
                      {["Oyente","Teléfono","Edad","Localidad","Género","Participaciones","Estado"].map(h=>(
                    ...
                    {oyentes.filter(o=>{
                      ...
                      <tr key={o.id} className="row" style={{borderBottom:`1px solid ${C.border}`,cursor:"pointer"}} onClick={()=>nav(V.OY_DET,o.id)}>
                        ...
                        <td style={{padding:"11px 14px"}}>
                          {isRecent(o)?<span className="tag-warn">⚠ Ganó hace {daysSince(o.ultimoWin)}d</span>:parseInt(o.totalWins)>0?<span className="tag-ok">🏆 {o.totalWins}</span>:<span style={{color:C.muted,fontSize:12}}>Sin premios</span>}
                        </td>
                      </tr>
                    ))}
                  </tbody>
        ...
        {view===V.OY_DET&&selOy&&(
          <div className="fade-in">
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:14}}>
              {[["📱 Teléfono / WhatsApp",selOy.telefono],["🪪 DNI",selOy.dni],["✉️ Email",selOy.email||"No registrado"],["🎂 Edad",selOy.edad?selOy.edad+" años":"No registrada"],["📍 Localidad",selOy.localidad||"No registrada"],["👤 Género",selOy.genero||"No registrado"]].map(([l,val])=>(
            ...
            <div className="card" style={{padding:18}}>
              ...
              {participaciones.filter(p=>p.oyenteId===selOy.id).map(p=>{
                const con=concursos.find(c=>c.id===p.concursoId); if(!con)return null;
                return(<div key={p.concursoId} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"9px 0",borderBottom:`1px solid ${C.border}`}}>
                  <div><div style={{fontWeight:600,fontSize:13}}>{con.nombre}</div><div style={{color:C.muted,fontSize:11}}>🎁 {con.premio}</div></div>
                  {con.ganadorId===selOy.id?<span className="tag-ok">🏆 Ganador</span>:con.estado==="activo"?<span className="tag-active">En curso</span>:<span className="tag-done">No ganó</span>}
                </div>);
              })}
            </div>
          </div>
        ...
        {view===V.OY_NEW&&(
          <div className="fade-in" style={{maxWidth:520}}>
            ...
            <button className="btn" onClick={addOyente} disabled={saving} style={{width:"100%",padding:13,marginTop:20}}>
        ...
        {view===V.CONS&&(
          <div className="fade-in">
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:20,gap:10,flexWrap:"wrap"}}>
              <Title t="Concursos" sub={`${concursos.length} en total`}/>
            ...
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              ...
              {concursos.map(c=>{
                ...
                return(
                  <div key={c.id} className="card row" onClick={()=>nav(V.CON_DET,c.id)} style={{padding:"16px 18px",cursor:"pointer",display:"flex",alignItems:"center",gap:14}}>
                    <div style={{flex:1}}>
        ...
        {view===V.CON_DET&&selCon&&(()=>{
          const partic=conPartic(selCon.id);
          const noInscrip=oyentes.filter(o=>!inCon(o.id,selCon.id));
          const [localBusq, setLocalBusq] = useState("");
          return(
            <div className="fade-in">
              <Back to={V.CONS}/>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16,flexWrap:"wrap",gap:10}}>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:4,flexWrap:"wrap"}}>
                    ...
                    {selCon.estado==="activo"?<span className="tag-active">Activo</span>:<span className="tag-done">Finalizado</span>}
                ...
                {selCon.estado==="activo"&&partic.length>0&&selCon.tipo==="sorteo"&&(
              ...
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14}}>
                {/* Participantes */}
                <div className="card" style={{padding:18}}>
                  <h3 style={{fontFamily:"'Barlow Condensed',sans-serif",fontSize:15,fontWeight:700,color:C.muted,textTransform:"uppercase",letterSpacing:1,marginBottom:14}}>Participantes ({partic.length})</h3>
                  {partic.length===0&&<p style={{color:C.muted,fontSize:13}}>Sin participantes aún.</p>}
                  {partic.map(o=>(
                    <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                      ...
                      <div style={{display:"flex",gap:4,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>
                        ...
                        {selCon.estado==="activo"&&selCon.tipo==="ganador_directo"&&selCon.ganadorId!==o.id&&(
                          <button className="btn-green" onClick={()=>marcarGanadorDirecto(o)} style={{padding:"3px 10px",fontSize:11,fontFamily:"'Nunito',sans-serif",textTransform:"none",letterSpacing:0}}>✓ Ganador</button>
                        )}
                      </div>
                    </div>
                  ))}
                ...
                {selCon.estado==="activo"&&(
                  <div className="card" style={{padding:18}}>
                    ...
                    <input className="inp" placeholder="Buscar..." value={localBusq} onChange={e=>setLocalBusq(e.target.value)} style={{marginBottom:10,padding:"8px 12px",fontSize:13}}/>
                    <div style={{maxHeight:280,overflowY:"auto"}}>
                      {noInscrip.filter(o=>!localBusq||o.nombre.toLowerCase().includes(localBusq.toLowerCase())||o.telefono.includes(localBusq)).map(o=>(
                        <div key={o.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:`1px solid ${C.border}`}}>
                          <div>
                          ...
                          <button className="btn-ghost" onClick={()=>inscribir(o.id,selCon.id)} style={{padding:"4px 10px",fontSize:12,flexShrink:0}}>+</button>
                        </div>
                      ))}
                      {noInscrip.length===0&&<p style={{color:C.muted,fontSize:13}}>Todos inscriptos.</p>}
                    </div>
                  </div>
                )}
              </div>
            ...
            {sorteoState.ganador&&!sorteoState.animando&&(
              <div className="win-in">
                ...
                {parseInt(sorteoState.ganador.totalWins)>1&&(
                  <div style={{background:C.warn+"11",border:`1px solid ${C.warn}44`,borderRadius:8,padding:"10px 20px",display:"inline-block",marginBottom:20}}>
                    <span style={{color:C.warn,fontSize:13,fontWeight:600}}>⚠️ Ganó {parseInt(sorteoState.ganador.totalWins)-1} veces anteriormente</span>
                  </div>
                )}
        ...
        {view===V.CON_NEW&&(
          <div className="fade-in" style={{maxWidth:520}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:14,marginBottom:14}}>
              <div>
                <Lbl>Fecha desde</Lbl>
                <input className="inp" type="date" value={conForm.fechaDesde} onChange={e=>setConForm(p=>({...p,fechaDesde:e.target.value}))}/>
              </div>
              <div>
                <Lbl>Fecha hasta</Lbl>
                <input className="inp" type="date" value={conForm.fechaHasta} onChange={e=>setConForm(p=>({...p,fechaHasta:e.target.value}))}/>
              </div>
            </div>
            ...
            <button className="btn" onClick={addConcurso} disabled={saving} style={{width:"100%",padding:13}}>
            ...
            {busqueda.length>=4&&(()=>{
              ...
              if(!found)return(
                <div className="card" style={{padding:28,textAlign:"center"}}>
                  ...
                  <p style={{color:C.muted,fontSize:14}}>No encontrado. <button className="btn" onClick={()=>nav(V.OY_NEW)} style={{padding:"8px 16px",marginLeft:8,fontSize:13}}>+ Registrar nuevo</button></p>
                </div>
              );
              return(
                <div className="card fade-in" style={{padding:20}}>
                  ...
                  {isRecent(found)&&(
                    <div style={{background:C.warn+"11",border:`1px solid ${C.warn}44`,borderRadius:8,padding:"12px 16px",display:"flex",gap:8,alignItems:"center"}}>
                      <span style={{fontSize:18}}>⚠️</span>
...
/* ─── CONFIG ──────────────────────────────────────────────────────────────── */
const GAS_URL = "https://script.google.com/macros/s/AKfycbxq2nwolBvf5hAOzDqf3-wiobtTxbPDmsc_XLfRR-TdIFYveWbREmk4dpBSn_iAa779/exec";
